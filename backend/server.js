...
// ─── AI (ai_providers bridge przez python3 -m ai_providers ask) ────────
// UWAGA: dashboard bywa wystawiony publicznie (share.sh / Cloudflare tunnel).
// Endpointy AI są DOMYŚLNIE WYŁĄCZONE i wymagają osobnego AI_API_KEY,
// inaczej każdy obcy z linkiem paliłby darmowe tokeny LLM.
const AI_API_KEY = process.env.AI_API_KEY || '';
const AI_RATE_WINDOW = 60000;
const AI_RATE_MAX = parseInt(process.env.AI_RATE_MAX || '10', 10); // req/min
const AI_TIMEOUT_MS = parseInt(process.env.AI_TIMEOUT_MS || '150000', 10);
const AI_RATE_COUNTS = new Map();

// timing-safe porownanie stringow (ochrona przed timing attack na kluczu)
function safeEq(a, b) {
  if (typeof a !== 'string' || typeof b !== 'string') return false;
  const ab = Buffer.from(a);
  const bb = Buffer.from(b);
  if (ab.length !== bb.length) {
    // porownaj z samym soba zeby czas byl staly mimo roznej dlugosci
...
function aiRateLimit(req, res, next) {
  ...
  let entry = AI_RATE_COUNTS.get(key);
  if (!entry || now > entry.reset) {
    entry = { count: 0, reset: now + AI_RATE_WINDOW };
    AI_RATE_COUNTS.set(key, entry);
  }
  entry.count++;
  if (entry.count > AI_RATE_MAX) {
    return res.status(429).json({ error: `AI rate limit: max ${AI_RATE_MAX}/min` });
  }
  next();
}
...
function requireAiKey(req, res, next) {
  if (!AI_API_KEY) {
    return res.status(503).json({
      error: 'AI disabled: ustaw AI_API_KEY w backend/.env (osobny od API_KEY)',
    });
  }
  if (!safeEq(req.headers['x-ai-key'], AI_API_KEY)) {
    return res.status(401).json({ error: 'Unauthorized: invalid or missing X-AI-Key header' });
  }
  next();
}

function aiAsk(payload) {
  // Most do wspolnego chainu ai_providers (pip install --user -e ~/Muzyka/projekty)
  return new Promise((resolve) => {
    const proc = execFile(
      'python3', ['-m', 'ai_providers', 'ask'],
      { timeout: AI_TIMEOUT_MS, maxBuffer: 2 * 1024 * 1024 },
      (err, stdout) => {
        if (!stdout || !String(stdout).trim()) {
          return resolve({ success: false, error: err ? err.message : 'brak odpowiedzi' });
        }
        try {
          const line = String(stdout).trim().split('\n').filter((l) => l.startsWith('{')).pop();
          resolve(JSON.parse(line || '{}'));
        } catch (e) {
          resolve({ success: false, error: 'zly output mostu AI' });
        }
      }
    );
    try {
...
app.post('/api/ai/ask', requireAiKey, aiRateLimit, async (req, res) => {
  const prompt = String(req.body.prompt || '').trim();
  if (!prompt) return res.status(400).json({ error: "Pole 'prompt' wymagane" });
  const payload = {
    prompt: prompt.substring(0, 8000),
    system: req.body.system ? String(req.body.system).substring(0, 2000) : undefined,
    chat_only: req.body.chat_only !== false, // domyslnie tylko LLM-y
  };
  const result = await aiAsk(payload);
  if (!result.success) return res.status(502).json({ success: false, error: result.error });
  res.json({ success: true, data: result });
});

app.get('/api/ai/status', requireAiKey, (req, res) => {
  res.json({
    success: true,
    data: {
      enabled: Boolean(AI_API_KEY),
      rate_limit: `${AI_RATE_MAX}/min`,
